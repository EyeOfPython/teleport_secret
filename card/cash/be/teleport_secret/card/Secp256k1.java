...
public class Secp256k1 {
    ...
    // https://javacard.kenai.com/javadocs/connected/javacard/security/KeyAgreement.html#ALG_EC_SVDP_DH_PLAIN
    ...
    public static boolean checkCurveParameters(ECKey eckey, byte[] tmp, short tmpOffset) {
        eckey.getA(tmp, tmpOffset);
        if (0 != Util.arrayCompare(tmp, tmpOffset, SECP256K1, OFFSET_SECP256K1_a, (short)32))
            return false;
        eckey.getB(tmp, tmpOffset);
        if (0 != Util.arrayCompare(tmp, tmpOffset, SECP256K1, OFFSET_SECP256K1_b, (short)32))
            return false;
        eckey.getG(tmp, tmpOffset);
        if (0 != Util.arrayCompare(tmp, tmpOffset, SECP256K1, OFFSET_SECP256K1_G, (short)65))
            return false;
        eckey.getR(tmp, tmpOffset);
        if (0 != Util.arrayCompare(tmp, tmpOffset, SECP256K1, OFFSET_SECP256K1_R, (short)32))
            return false;
        eckey.getField(tmp, tmpOffset);
        if (0 != Util.arrayCompare(tmp, tmpOffset, SECP256K1, OFFSET_SECP256K1_P, (short)32))
            return false;
        return eckey.getK() == SECP256K1_K;
    }
}
